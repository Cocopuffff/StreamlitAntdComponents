...
const AntdTransfer = (props: TransferProp) => {
  ...
  const onChange = (
    ...
    Streamlit.setComponentValue(
      nextTargetKeys.map((x) => (return_index ? Number(x) : kv[Number(x)]))
    );
  };
  const onSearch = (dir: TransferDirection, value: string) => {
    Streamlit.setComponentValue(
      targetKeys.map((x: any) => (return_index ? Number(x) : kv[Number(x)]))
    );
  };
  const reset = () => {
    setDataSource(items);
    setTargetKeys(index);
    Streamlit.setComponentValue(
      props["index"].map((x: any) => (return_index ? x : kv[x]))
    );
  };
      ...
      <LabelWrap
        ...
        children={
          <Transfer
            ...
            footer={
              typeof reload == "string" || String(reload) === "true"
                ? renderFooter
                : undefined
            }
          />
